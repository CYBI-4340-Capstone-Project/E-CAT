...
def saveTable(table, tableName, mycaption, mylabel):
    ...
    featFile.write(table.to_latex(column_format='c'*table.columns.size, index=False,
                              caption=r'F1 score of each data set\'s best model on other data sets \[ {0}\]'.format(name.replace("_"," ")),
                              label=mylabel, position="H")
                  )
    featFile.close()
...
def buildDataset(pcapTypeNum, maxNumFiles, datasetTypeNum, filepath):
    ...
    # Drop Label_x and rename Label_y to Label
    if 'Label_x' in full_data.columns and 'Label_y' in full_data.columns:
        full_data.drop(columns=['Label_x'], inplace=True)
        full_data.rename(columns={'Label_y': 'Label'}, inplace=True)  
    ...
    zeroVar = numeric_cols[(full_data[numeric_cols].var() == 0).values]
