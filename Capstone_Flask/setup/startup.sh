...
sudo timedatectl set-timezone America/Chicago  # Change "America/(your/timezone)"
...
if [ -f "$DATA_DIR/startup_complete.txt" ]; then
  echo "Startup script already run, exiting."
  cd "$BASE_DIR"
  ...
  pwd >> "LOG_FILE" 2>&1
  source /env/bin/activate && gunicorn app:app >> "$LOG_FILE" 2>&1
  if [ $? -ne 0 ]; then
...
sudo chown -R "$NEW_USERNAME":"$NEW_USERNAME" /home/"$NEW_USERNAME"/E-CAT
...
sudo touch "$DATA_DIR/startup_complete.txt"
...
pip install xgboost
sudo apt-get install gcc g++ libpcap-dev libssl-dev python3-dev zlib1g-dev -y
...
sudo mkdir -p "$BASE_DIR/Databases/CICFlowMeter-3.0/bin/data" >> "$LOG_FILE"
sudo mkdir -p "$BASE_DIR/Databases/CICFlowMeter-3.0/bin/data/in" >> "$LOG_FILE"
sudo mkdir -p "$BASE_DIR/Databases/CICFlowMeter-3.0/bin/data/out" >> "$LOG_FILE"
#sudo mkdir -p "$BASE_DIR/Databases/OG_PCAPs" >> "$LOG_FILE"
sudo mkdir -p "$BASE_DIR/logs/$TODAY_DATE" >> "$LOG_FILE"
#sudo chown -R capstone4340-admin:capstone4340-admin "$BASE_DIR" >> "$LOG_FILE"
sudo chmod -R 777 "$BASE_DIR" >> "$LOG_FILE"
...
sudo chmod 777 "$BASE_DIR/Databases/CICFlowMeter-3.0/bin/CICFlowMeter"
cd "$BASE_DIR/setup"
